<html>
<head>
<title>Issue Bakijai Notice</title>
</head>
...
if (i==1)
{
myform.action="Notice.php?tag=2&ptype=0&mtype="+i;
myform.submit();
}

if(i==101 || i==102 || i==2) //postback on textbox
{    
myform.action="Notice.php?tag=2&ptype=1&mtype="+i;    
myform.submit();
}
...
var m=myform.Notice_type.value;
...
{
myform.action="Insert_bakijai_casedate.php?Nt="+m;
myform.submit();
}
}
else
...
$objBakijai_casedate->setCondString("Case_id=".$mvalue[0]);
$notrow=$objBakijai_casedate->getAllRecord();
}
...
<input type=hidden size=8 name="Day" id="Day" value="<?php echo $mvalue[$i]; ?>" onfocus="ChangeFocus('Paid_today')"  onblur="ChangeColor('Day',2)" readonly>
<font color=black size=2 face=arial><b><?php echo $mvalue[$i]; ?></b></font>
</td>
</tr>
<?php $i++; //Now i=2
$objN=new Noticetype();
$objN->setCondString("Code not in(5,6)");
$row=$objN->getRow();
?>
<tr>
...
{
if($mvalue[2]==7)
$file="GenerateRajahAdalat.php";
else
$file="GenerateShowCause.php";
?>
<a href="<?php echo $file;?>?id=<?php echo $_SESSION['oldid'];?>"   target=_blank>Generate Show Cause Notice for Case ID-<?php echo $_SESSION['oldid'];?></a>
...
<select name="kj">
<?php
for($ii=0;$ii<count($notrow);$ii++)   
{
$det="Issued ".$notrow[$ii]['Notice_Detail']." on ".$objUtility->to_date($notrow[$ii]['Entry_date'])." for ".$objUtility->to_date($notrow[$ii]['Next_date']);
?>   
<option value="0"><?php echo $det; 
}//for
}//if $tot>0
}
?></select>
...
<tr>
<td align=right bgcolor=#FFFFCC><font color=black size=2 face=arial>Type Name</font></td><td align=left bgcolor=#FFFFCC colspan="3">
<input type=text size=40 name="Fathername" id="Fathername" value="<?php echo $father; ?>" style="font-family: Arial;background-color:white;color:black; font-size: 14px" maxlength=50 onfocus="ChangeColor('Fathername',1)"  onblur="ChangeColor('Fathername',2);transE('Fathername','Father')" onkeyup="trans('Fathername','Father_ass')">
<font color=green size=2 face=arial>
This Text will be converted to Unicode and Proper English
</font>
</td>
