...
function EnuButton(obj)
{
    document.getElementById(obj).disabled = false;
    document.getElementById(obj).style.backgroundColor = '#66CC66';
}
...
// Something went wrong
    ...
    ajaxRequest.onreadystatechange = function() {
        if (ajaxRequest.readyState == 4) {
            ...
            var text = ajaxRequest.responseText;
            console.log(text);
            var ind = text.indexOf("{", 0); // Search Position index of  { as first occurence
            var mylength = parseInt(text.length);
            var text = text.substr(ind, mylength);// 
...
function Push_Item_in_select_box(obj, Arr)
{
    ...
    var myopt = '<option value=' + "0" + '>' + "Select" + '</option>';
