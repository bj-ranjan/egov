<html>
<head>
<title>Entry Form for branch_section</title>
</head>
...
myform.Branch_code.value=mvalue;

var a=myform.Branch_code.value ;//Primary Key
if ( isNaN(a)==false && a!="")
{
myform.action="Form_branch_section.php?tag=2&ptype=0";
myform.submit();
}
}
...
var a=myform.Branch_code.value ;// Primary Key
var b=myform.Branch_name.value ;
var b_length=parseInt(b.length);
if ( isNumber(a)==true   && notNull(b) && validateString(b) && b_length<=40)
{
myform.action="Insert_branch_section.php";
myform.submit();
}
else
alert('Invalid Data');
...
$mvalue[2]=0;
$mvalue[0]=$objBranch_section->MaxBranch_code();
}
else
{
...
$_SESSION['update']=0;
$_SESSION['msg']="";
// Call $objBranch_section->MaxBranch_code() Function Here if required and Load in $mvalue[0]
$mvalue[0]=$objBranch_section->MaxBranch_code();//Branch_code
$mvalue[1]="";//Branch_name
...
$pkarray[0]=$_POST['Branch_code'];
else
$pkarray[0]=0;
$objBranch_section->setBranch_code($pkarray[0]);
if ($objBranch_section->EditRecord()) //i.e Data Available
{ 
if ($ptype==0) //Post Back on Edit Button Click
{
$mvalue[0]=$objBranch_section->getBranch_code();
$mvalue[1]=$objBranch_section->getBranch_name();
$mvalue[2]=0;//last Select Box for Editing
} //ptype=0
$_SESSION['update']=1;
} 
...
<form name=myform action=insert_branch_section.php  method=POST >
<tr><td colspan=2 align=Center bgcolor=#ccffcc><font face=arial size=3>Entry/Edit Form for Branch_section<br></font><font face=arial color=red size=2><?php echo  $_SESSION['msg'] ?></font></td></tr>
<?php $i=0; ?>
<tr>
<td align=right bgcolor=#FFFFCC><font color=black size=2 face=arial>Branch_code</font></td><td align=left bgcolor=#FFFFCC>
<input type=text size=8 name="Branch_code" id="Branch_code" value="<?php echo $mvalue[$i]; ?>" onfocus="ChangeColor('Branch_code',1)"  onblur="ChangeColor('Branch_code',2)" onchange=direct1() readonly>
<font color=red size=4 face=arial><b>*</b></font>
</td>
</tr>
<?php $i++; //Now i=1?>
<tr>
<td align=right bgcolor=#FFFFCC><font color=black size=2 face=arial>Branch_name</font></td><td align=left bgcolor=#FFFFCC>
<input type=text size=40 name="Branch_name" id="Branch_name" value="<?php echo $mvalue[$i]; ?>" style="font-family: Arial;background-color:white;color:black; font-size: 12px" maxlength=40 onfocus="ChangeColor('Branch_name',1)"  onblur="ChangeColor('Branch_name',2)">
