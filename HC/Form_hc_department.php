<html>
<head>
<title>Entry Form for hc_department</title>
</head>
...
if ( isNaN(a)==false && a!="")
{
myform.action="Form_hc_department.php?tag=2&ptype=0";
myform.submit();
}
}
...
function res()
{
window.location="Form_hc_department.php?tag=0";
}
//END JAVA
</script>
<body>
...
$mvalue[2]=0;
$mvalue[0]=$objHc_department->MaxCode();
}
else
{
$mvalue[0]=$objHc_department->MaxCode();
...
$_SESSION['update']=0;
$_SESSION['msg']="";
// Call $objHc_department->MaxCode() Function Here if required and Load in $mvalue[0]
$mvalue[0]=$objHc_department->MaxCode();//Code
$mvalue[1]="";//Name
...
$pkarray[0]=0;
$objHc_department->setCode($pkarray[0]);
if ($objHc_department->EditRecord()) //i.e Data Available
{ 
if ($ptype==0) //Post Back on Edit Button Click
{
$mvalue[0]=$objHc_department->getCode();
$mvalue[1]=$objHc_department->getName();
$mvalue[2]=0;//last Select Box for Editing
} //ptype=0
$_SESSION['update']=1;
} 
...
<form name=myform action=insert_hc_department.php  method=POST >
<tr>
<td colspan=2 align=Center bgcolor=#66CC66>
<font face=arial size=3>Entry/Edit Form for Hc_department<br></font>
...
$mystyle="font-family: Arial;font-weight:bold;background-color:white;color:black;font-size: 12px";
?>
<input type=text size=50 name="Name" id="Name" value="<?php echo $mvalue[1]; ?>" style="<?php echo $mystyle;?>"  maxlength=100 onfocus="ChangeColor('Name',1)"  onblur="ChangeColor('Name',2)">
