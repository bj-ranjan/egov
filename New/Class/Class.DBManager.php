...
class DBManager extends BusinessMethod {
     ...
     public function SystemManagerIP() {
        $ok = false;
        
            $ip = $this->MyClientIP();

        $cond = " roll=0 and uid='root' ";

        $aip = $this->FetchColumn($this->SchemaDot() . "pwd", "allowed_ip", $cond, "A");
//echo $uid." ". $ip."  ".$aip;
    ...
    public function genBootStrapDataGridOnValueList($title, $headlist, $align, $ValueList, $width, $records = 0, $RowColor = array()) {
        ...
        if ($cnt > 0) {
            ...
            echo "<thead>";
            if (strlen($title) > 0) {
                echo "<tr><td align=center  colspan=" . ($numcol + $tag) . "><font face=arial size=" . $this->TableTitleFont . "><b>" . $title . "</b></td></tr>";
            }
            echo "<tr>";

            if ($this->TableAutoSerialColumnRequired == 1)
                echo "<td align=center bgcolor=" . chr(34) . $this->TableHeadColor . chr(34) . "><font face=arial size=" . $this->TableHeadFont . "><b>SlNo</b></td>";

            for ($i = 0; $i < count($headlist); $i++) {
                echo "<td align=center bgcolor=" . chr(34) . $this->TableHeadColor . chr(34) . "><font face=arial size=" . $this->TableHeadFont . "><b>" . $headlist[$i] . "</b></td>";
            }
            echo "</tr></Thead>";
//end header
        }//cnt>0
...
//$tcol=$this->checkList($ind,$ValueList,$numcol);
           ...
           // if (isset($RowColor[$ind]))
            ...
            if ($this->TableAutoSerialColumnRequired == 1) {
                if ($ind <= ($records - 1))
                    echo "<td height='" . $this->TDHeight . "' align=center " . $bgcol . ">" . $sl . "</td>";
                else
                    echo "<td   height='" . $this->TDHeight . "'   align=center " . $bgcol . "></td>";
            }
            ...
            for ($i = 0; $i < $numcol; $i++) {
                ...
                if (isset($ValueList[$ind][$i]))
                    ...
                    $fld = "&nbsp;";
                echo "<td align=" . $malign . $bgcol . ">" . $fld . "</td>";
            }
            echo "</tr>";
        }//end Row browse
        echo "</table>";
...
//echo $uid." ". $ip."  ".$aip;
    ...
    public function FetchRecordUsingIndexValueBySQL($sql, $cols = 1) {
        $temp = array();
        $row = $this->FetchRecords($sql);
        for ($i = 0; $i < count($row); $i++) {
            switch ($cols) {
                case 2:
                    $code = $row[$i][0];
                    $code1 = isset($row[$i][1]) ? $row[$i][1] : $code;
                    $val = isset($row[$i][2]) ? $row[$i][2] : $code1;
                    $temp[$code][$code1] = $val;
                    break;
                ...
                case 3:
                    $code = $row[$i][0];
                    $code1 = isset($row[$i][1]) ? $row[$i][1] : $code;
                    $code2 = isset($row[$i][2]) ? $row[$i][2] : $code1;
                    $val = isset($row[$i][3]) ? $row[$i][3] : $code2;
                    $temp[$code][$code1][$code2] = $val;
                    break;

                default:
                    $code = $row[$i][0];
                    $val = isset($row[$i][1]) ? $row[$i][1] : $code;
                    $temp[$code] = $val;
                    break;
            }
        }
...
//generate Captcha
    ...
    public function genSqlQuery($table, $fldlist, $cond) {
    ...
    public function ExecuteBatchData($Table, $FldList, $ValueList, $Packet, $showerror = false) {
        ...
        for ($i = 0; $i < count($ValueList); $i++) {
            ...
            for ($j = 0; $j < $numCol; $j++) {
                ...
                if ($quote == "Y" && strtoupper($ValueList[$i][$j]) != "NULL")
...
//Handle remaining Row
     ...
     public function ExecuteBatchDataComplete($Table, $FldList, $ValueList, $Packet, $showerror = false) {
        $numCol = count($FldList);
        $numRow = count($ValueList);
        $sql = "";
        $Table=$this->SchemaDotTable($Table);
...
//end JSON Object
    ...
    public function TableBusy($table = "X") {
        ...
        $table=$this->SchemaDotTable($table);
        $ip = $this->MyClientIP();
        $slno = $this->FetchColumn("lockque", "slno", "ip='" . $ip . "' and Table_name='" . $table . "'", 3500);

        $rec = $this->FetchColumn("lockque", "slno", "Table_name='" . $table . "' and lock_status='Y' and slno<" . $slno, 0);

        $str = $ip . " " . $this->returnSql . " " . $rec . "<br>";

        //$this->AppendF("TableBusy.txt", $this->MyClientIP()."-".$slno." rec-".$rec);
