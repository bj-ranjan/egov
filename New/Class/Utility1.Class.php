...
class Utility extends DBManager {
    ...
    public function Utility($simple = 0) {    //Keep Parameter as '0' for High Security  and prevention of Bypass
        $roll = -1;
        parent::__construct();
            }
...
//CriticalAllowed
    ...
    public function UserPresent() {
        ...
        if (isset($_SESSION['uid']))
            ...
            $uid = "-";
        $sql = "update " . $this->SchemaDot() . "userlog set Log_time_out='" . date('H:i:s') . "' where Session_id=" . $sid . " and Uid='" . $uid . "'";
        $this->Execute($sql);
    }
...
//Use your Business Logic  to Verify Every page(Use/Password,Ip etc))
        ...
        $rec = $this->FetchRecords("select pwd,roll from " . $this->SchemaDot() . "pwd where uid='" . $this->CurrentUid() . "'");
        if (count($rec) > 0) {
            $pwd = $rec[0][0];
            $roll = $rec[0][1];
        }
...
//validateText
    ...
    public function saveErrorLog() {
        $fname = $this->ApplicationFolder() . "/Log/Error" . date('dmY') . ",txt";
        $this->AppendF($fname, $this->MyClientIP());
        $this->AppendF($fname, $this->DeoName() . " " . $this->CurrentUid());
        $this->AppendF($fname, $this->Error());
        $this->AppendF($fname, '');
    }
