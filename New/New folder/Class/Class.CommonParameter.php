...
//$patern="/^[1-9]{1,4}[a-zA-Z]{0,4}$/";  //Start With 1-9 / 1o or 4 character and end with any alphabet or null
    //Bit handling
    private $State = "Assam";
    private $idletime = 9000;
    public $CentralDatabase = "deo";
    public $SerialAss = false;
    public $AutoBackupInterval = 10800; //3 Hours 60X60x3
    ...
    public $False = "false";
    public $True = "true";
    public $BooleanTrue = "'t'"; //boolean field
    public $BooleanFalse = "'f'"; //boolean field
    ...
    public $BackEndCode = 1;
    public $pkList;
    public $fkList;
    public $PdfOutput = true;
    public $HideObject = false; //if true ,Hides some classes on logout and show on login
    private $Table_schema;
    private $IP = array("10.177.92.2", "10.177.92.55"); //Generally Developers Machine IP for Critical Operation
...
//private $patern="^[1-9]{1,4}[a-zA-Z]{0-4}$";  //Start With 1-9 / 1o or 4 character and end with any alphabet or null

    private $FileExtension = array(1 => "GIF", 2 => "JPG", 3 => "PNG", 4 => "SWF", 5 => "PSD", 6 => "BMP");

    public function Frame() {
        $frame = isset($_SESSION['Frame']) ? $_SESSION['Frame'] : '0';
        return($frame);
    }

    public function EncryptVersion() {
    ...
    public function Salt() {
        $salt = rand(10501, 98756);
        $_SESSION['salt'] = $this->Sanitize_Number($salt);
        return($salt);
    }

    public function DistrictDatabase() {
        $database = isset($_SESSION['DistrictDatabase']) ? $_SESSION['DistrictDatabase'] : '';
        return($database);
    }

    public function ValidVehicleRegNo($number) {
        $last = substr($number, -4);
        if (preg_match("/^[a-zA-Z]{2}/", $number) && is_numeric($last) && strlen($number) > 7)
            return(true);
        else
            return(false);
    }

    public function ValidMobile($number) {
        $digit1 = substr($number, 0, 1);
        if (strlen($number) == 10 && is_numeric($number) && ($digit1 == "9" || $digit1 == "8" || $digit1 == "7"))
            return(true);
        else
            return(false);
    }
    ...
    public function CheckRequestMethod() {
        if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST')
            ...
            $url = Config::getBase_url() . "login.htm";
            $this->AlertNRedirect("Bad Source", $url);
            return(false);
        }
    }

    public function CSRF_TokenWithHiddenBox() {
        $temp = isset($_SESSION['csrf_token']) ? $_SESSION['csrf_token'] : '#x';
        $_SESSION['prev_csrf_token'] = $this->Sanitize_Special($temp);
        $num = md5(rand(1000, 3000) . "Myname&");
        $this->genHiddenBox("csrf_token", $num);
        $_SESSION['csrf_token'] = $this->Sanitize_Special($num);
        //echo "Session Stored ".$_SESSION['csrf_token']."<br>";
    }

    public function CheckToken($type = 1, $return = true) {   //1-Present and any- preveous
        $prev_token = isset($_SESSION['prev_csrf_token']) ? $_SESSION['prev_csrf_token'] : '--';
        $pres_token = isset($_SESSION['csrf_token']) ? $_SESSION['csrf_token'] : '--'; // $_SESSION['csrf_token'] 
        $posted_token = isset($_POST['csrf_token']) ? $_POST['csrf_token'] : 'xx';

        $token = ($type == 1) ? $pres_token : $prev_token;
        if ($token === $posted_token)
            $res = true;
        else
            $res = false;
        if ($res == false) {
            ...
            if ($return == true) {
                ...
                $this->AlertNRedirect("Bad Source " . $token . "=" . $posted_token, $url);
            }
        }
        return($res);
    }

    public function Image_Description($path, &$extension, &$width = 0, &$height = 0) {
        $type = "";
        if (file_exists($path)) {
            $A = getimagesize($path);
            $width = $A[0];
            $height = $A[1];
            $extension = $A[2];
            $type = isset($this->FileExtension[$extension]) ? $this->FileExtension[$extension] : '';
        }
        return($type);
    }
    ...
    public function GenerateCaptcha() {
        ...
        $List[0] = rand(9, 18); //CAP
        $List[1] = rand(27, 32); //Small
        $List[2] = rand(0, 8); // number

        $List[3] = rand(19, 26); //CAP
        $List[4] = rand(33, 42); //Small

        $captcha = "";
        $used = array('N', 'N', 'N', 'N', 'N');
        for ($i = 0; $i < 5; $i++) {
            $index = rand(0, 4);
            while ($used[$index] == "Y") {
                $index++;
    ...
    public function Divide($Value, $Divideby, &$Resedue = 0) {
        $a = 0;
        if ($Divideby > 0) {
            $Resedue = $Value % $Divideby;
            $a = ($Value - $Resedue) / $Divideby;
        }//$Divideby>0
        return($a);
    }
    ...
    public function AllowedFile() {
        $files = array();
        if (isset($_SESSION['AllowedFile']))
            $files = $_SESSION['AllowedFile'];
...
//Use your Business Logic  to Verify Every page(Use/Password,Ip etc))
    ...
    public function BackUpDue($ind) {
        $path = $this->ApplicationFolder();
        $fname = $path . "/TextFile/LastBackup" . $ind . ".txt";
...
//backupdue
    ...
    public function SaveQueryAsTextFile($sql, $sub = "") {
        ...
        if (strlen($sub) == 0)
            ...
            $file = $this->ApplicationFolder() . "/log/" . $sub . "/" . date('dmY') . "query.sql";
    ...
    public function ElectionName() {
        return(isset($_SESSION['ElectionName']) ? $_SESSION['ElectionName'] : '');
    }
    ...
    public function DeoCode() {
        if (isset($_SESSION['Deo_Code']))
            return($_SESSION['Deo_Code']);
        else
            return("0");
    }

    public function DeoName() {
        if (isset($_SESSION['Deo_Name']))
            return($_SESSION['Deo_Name']);
        else
            return("NA");
    }
    ...
    public function DivClass($id, $align, $class) {
        $id = $this->Sanitize_AlphaNumeric($id, 20);
        $align = $this->Sanitize_Number($align, 1);
        $class = $this->Sanitize_AlphaNumeric($class, 20);

        $mal = $this->Sanitize_Alphabet($this->mAlign[$align], 8);
...
//end function
//HTML BOX
    public function genDataGridNew($title, $headlist, $align, $ValueList, $width, $records = 0, $type = 0) {
        $tAlign = array(1 => 'Left', 2 => 'center', 3 => 'right');

        if ($records == 0)
            $records = count($ValueList);
        ...
        $width = $this->Sanitize_Number($width, 3);
...
//$tcol=$this->checkList($ind,$ValueList,$numcol);
            ...
            if ($this->TableAutoSerialColumnRequired == 1) {
                if ($ind <= ($records - 1))
                    echo "<td height='" . $this->TDHeight . "' align='center' " . $mstyle . ">" . $sl . "</td>";
                else
                    echo "<td height='" . $this->TDHeight . "'  align='center' " . $mstyle . ">&nbsp;</td>";
            }
    ...
    public function genDataGridOnValueList($title, $headlist, $align, $ValueList, $width, $records = 0, $RowColor = array()) {
        $tAlign = array(1 => 'Left', 2 => 'center', 3 => 'right');
...
//$tcol=$this->checkList($ind,$ValueList,$numcol);
            ...
            echo "<tr>";
            $bgcol = "";
            if (isset($RowColor[$ind]))
                $bgcol = " bgcolor='" . $RowColor[$ind] . "'";

            if ($this->TableAutoSerialColumnRequired == 1) {
                if ($ind <= ($records - 1))
                    echo "<td height='" . $this->TDHeight . "' align=center " . $bgcol . "><font face=arial size=" . $this->TableBodyFont . ">" . $sl . "</td>";
                else
                    echo "<td   height='" . $this->TDHeight . "'   align=center " . $bgcol . "><font face=arial size=2>&nbsp;</td>";
            }
            ...
            for ($i = 0; $i < $numcol; $i++) {
                ...
                if (isset($ValueList[$ind][$i]))
                    ...
                    $fld = "&nbsp;";
                echo "<td align=" . $malign . $bgcol . "><font face=arial size=" . $this->TableBodyFont . ">" . $fld . "</td>";
            }
            echo "</tr>";
        }//end Row browse
        echo "</table>";
...
//genDataGrid
    ...
    public function genDatePicker($Fld, $level) {
        $Fld = $this->Sanitize_AlphaNumeric($Fld, 20);
        $level = $this->Sanitize_Number($level);
        $path = "";
        if ($level == 1)
...
//genDatePicker
    ...
    public function CheckBox($id, $val, $bcol, $fcol, $font, $function, $mandatory) {
        ...
        $val = $this->Sanitize_Special($val, 20);
...
//genCheckbox
    ...
    public function genInputBox($id, $val = "", $size = 10, $maxlength = 10, $bcol = "white", $fcol = "black", $font = 12, $function = "", $mandatory = 0) {
        $id = $this->Sanitize_Special($id, 30);
        $val = $this->Sanitize_Special($val);
        $bcol = $this->Sanitize_Special($bcol, 10);
        $fcol = $this->Sanitize_Special($fcol, 10);
        $font = $this->Sanitize_Number($font, 2);
        $size = $this->Sanitize_Number($size, 3);
        $maxlength = $this->Sanitize_Number($maxlength, 3);
        $function = $this->Sanitize_Special($function, 30);
        $mandatory = $this->Sanitize_Number($mandatory, 1);
...
//genInputBox
    public function genCSSButton($id, $val, $width, $colortag, $font, $type, $function) {
        ...
        $font = $this->Sanitize_Number($font, 2);
        $width = $this->Sanitize_Number($width, 3);
        $type = $this->Sanitize_Number($type, 3);
        $colortag = $this->Sanitize_Number($colortag, 3);
...
//echo $classname."<br>";
    ...
    public function genButton($id, $val, $pix, $bcol, $fcol, $font, $function) {
        ...
        $fcol = $this->Sanitize_Special($fcol, 10);
        $font = $this->Sanitize_Number($font, 2);
        $pix = $this->Sanitize_Number($pix, 3);
...
//$mystyle="font-family: Arial;background-color:".$bcol.";color:".$fcol.";font-size:".$font."px;width:".$pix."px";
        ...
        $fcol = $this->Sanitize_Special($fcol, 10);
        $font = $this->Sanitize_Number($font, 2);
        $row = $this->Sanitize_Number($row, 1);
        $col = $this->Sanitize_Number($col, 2);
        $mandatory = $this->Sanitize_Number($mandatory, 1);
    ...
    public function returnTextArea($id, $val, $row, $col, $function) {
...
//$mystyle="font-family: Arial;background-color:".$bcol.";color:".$fcol.";font-size:".$font."px;width:".$pix."px";
        ...
        $mystyle = "font-family: Arial;background-color:" . $bcol . ";color:" . $fcol . ";font-size:" . $font . "px;padding: 4px 4px 4px 4px;border-radius: 4px;";
        $str = "<textarea  name=" . chr(34) . $id . chr(34) . "  id=" . chr(34) . $id . chr(34) . " rows=" . $row . " cols=" . $col . " style=" . chr(34) . $mystyle . chr(34) . " onfocus=" . chr(34) . "ChangeColor('$id',1)" . chr(34) . "  onblur=" . chr(34) . "ChangeColor('$id',2)" . chr(34) . " " . $place . " " . $function . ">";
        $str.=$val;
        $str.= "</textarea>";
        return($str);
    }
    ...
    public function genHiddenBox($id, $val) {
        $id = $this->Sanitize_Special($id, 20);
        $val = $this->Sanitize_Special($val, 100);
    ...
    public function ReturnTDText($property) {
        ...
        if (isset($property['align'])) {
            if ($align == 4) //Justified
                $text = "<div align=" . chr(34) . "justify" . chr(34) . ">" . htmlentities($text) . "</div>";
        }
...
//GenSelectBoxByValuarray
    ...
    public function AlertNRedirect($a, $page) {
        ...
        $a = str_replace("'", "", $a);
        if (strlen($a) > 0)
            echo "alert('" . $a . "');//Make an alert\n";
        if (strlen($page) > 0)
            echo "document.location.href=" . chr(34) . $page . chr(34) . ";//Redirect\n";
        echo "</script>";
    }
    ...
    public function OpenWindow($a, $page, $type) {
        ...
        if (strlen($a) > 0)
            echo "alert('" . $a . "');//Make an alert\n";
        if (strlen($page) > 0)
            echo "window.open(" . chr(34) . $page . chr(34) . "," . chr(34) . $type . chr(34) . ");//Redirect\n";
        echo "</script>";
    }
...
//  ^ is used to say that staring with  and $ - ending with
    ...
    public function TextHeader($msg, $width, $align, $bcol, $fcol, $font) {
        $align = $this->Sanitize_Number($align, 3);
        //$msg = $this->Sanitize_Special($msg, 200);
        $bcol = $this->Sanitize_Special($bcol, 10);
        $fcol = $this->Sanitize_Special($fcol, 10);
        $font = $this->Sanitize_Number($font, 2);
        $width = $this->Sanitize_Number($width, 3);
    ...
    public function ButtonHeader($msg, $font, $fun, $dark = 0) {

        $msg = $this->Sanitize_Special($msg, 60);
        $font = $this->Sanitize_Number($font, 2);
        $fun = $this->Sanitize_Special($fun, 30);
        ...
        if ($dark == 0) {
            $msg = "    " . $msg . "    ";
            //  echo "<input type=button class=" . chr(34) . "button-pink-marron button-font" . $font . chr(34) . " value=" . chr(34) . $msg . chr(34) . $fun . ">";
            echo " <table border='0' width='50%' align='center'>";
            echo "        <tr><td>";
            echo "    <div class='background4' align='center'>";
            //echo "    <div class='button-green-blue' align='center'>";

            echo "   <h3>    ";
            echo $msg;
            echo "   </h1></div> </td></tr> </table></div>";
        }
    }
...
//NEW HTML BOX
    public function newInputBox($property) {
        ...
        if ($this->DefaultMark == "Double") //double quote
            $str = "<input type=" . $text . " value=" . chr(34) . $val . chr(34) . "  name=" . chr(34) . $id . chr(34) . "  id=" . chr(34) . $id . chr(34) . " autocomplete=" . chr(34) . "off" . chr(34) . "    size=" . $size . $maxlength . " style=" . chr(34) . $mystyle . chr(34) . " onfocus=" . chr(34) . "ChangeColor('$id',1)" . chr(34) . "  onblur=" . chr(34) . "ChangeColor('$id',2)" . chr(34) . " " . $place . " " . $function . ">";
        else                                                            //autocomplete='off'
            $str = "<input type=" . $text . " value='" . $val . "'  name='" . $id . "'  id='" . $id . "'  autocomplete='off'   size=" . $size . $maxlength . " style='" . $mystyle . "'" . $place . " " . $function . ">";
...
//echo "select name=" . $id . "  id=" . $id . $size . " style=" . chr(34) . $mystyle . chr(34) . " " . $function;
        ...
        //echo "Inside Class".count($ValueList);
    ...
    public function CheckBoxWithValue($id, $val, $check, $function) {
        $id = $this->Sanitize_Special($id, 20);
        $val = $this->Sanitize_Special($val, 3);
        $function = $this->Sanitize_Special($function, 30);
    ...
    public function genFileBox($id, $size) {
        $id = $this->Sanitize_Special($id, 20);
        $size = $this->Sanitize_Special($size, 3);
...
//Handle remaining Row
    ...
    public function Return_JSON_Object_For_SelectBox($List, $SelecVal, $Default, $ExtraHeader = array()) { //This Return Value as Object is to be Parsed using JSON.parse()
        $List = $this->SanitizeArray($List);
        $SelecVal = $this->Sanitize_Special($SelecVal, 50);
        $ValueList = $List;

        $a = 0;
        if ($Default > 0 || $Default == true)
            $a++;

        echo "{" . chr(34) . "BoxSize" . chr(34) . ":" . chr(34) . (count($List) + $a) . chr(34) . "";
        echo "," . chr(34) . "SelectVal" . chr(34) . ":" . chr(34) . $SelecVal . chr(34) . "";

        foreach ($ExtraHeader as $index => $value) {
            // $index=$this->Sanitize_Special($index,30);
            // $value=$this->Sanitize_Special($value,30);
            echo "," . chr(34) . $index . chr(34) . ":" . chr(34) . $value . chr(34) . "";
        }

        echo "," . chr(34) . "Code" . chr(34) . ":[";

        if ($Default > 0 || $Default == true) {
            echo "{" . chr(34) . "Opt" . chr(34) . ":" . chr(34) . htmlentities($this->DefaultOpt) . chr(34);
            echo "," . chr(34) . "Detail" . chr(34) . ":" . chr(34) . htmlentities($this->DefaultOptDetail) . chr(34) . "}";
        }

        for ($i = 0; $i < count($List); $i++) {
            if ($a > 0)
                echo ",";
            echo "{" . chr(34) . "Opt" . chr(34) . ":" . chr(34) . htmlentities($List[$i][0]) . chr(34);
            echo "," . chr(34) . "Detail" . chr(34) . ":" . chr(34) . htmlentities($List[$i][1]) . chr(34) . "}";
            $a++;
        }//for loop
        echo "]}"; //  End Code 
...
$file=$ip.".Json";
      ...
      $this->tWriteF($file, "{");
        ...
        for ($i = 0; $i < $Found; $i++) {
            ...
            $this->tAppendF($file, chr(34) . $i . chr(34) . ":");
            echo "{";
            $this->tAppendF($file, "{");
            for ($j = 0; $j < $numcol; $j++) {
                $data = isset($ValueList[$i][$j]) ? $ValueList[$i][$j] : (isset($ValueList[$j]) ? $ValueList[$j] : '');
                ...
                $this->tAppendF($file, chr(34) . $head . chr(34) . ":" . chr(34) . $data . chr(34));
            }//inner for j
            echo "," . chr(34) . "Found" . chr(34) . ":" . chr(34) . $Found . chr(34);
            $this->tAppendF($file, "," . chr(34) . "Found" . chr(34) . ":" . chr(34) . $Found . chr(34));
            echo "," . chr(34) . "AlertMessage" . chr(34) . ":" . chr(34) . $this->Alert_Message_Through_JSON . chr(34);
            $this->tAppendF($file, "," . chr(34) . "AlertMessage" . chr(34) . ":" . chr(34) . $this->Alert_Message_Through_JSON . chr(34));
            echo "}";
            $this->tAppendF($file, "}");
        }// outer for i
        echo "}";
...
//function end
    ...
    public function GenerateCSV($File, $Head, $List, $delim = ";") {
    ...
    public function IntegerPart($amt) {
        $val = $amt;
        $ind = $this->inStr($amt, ".");
        if ($ind == 0)
            $val = 0;
        if ($ind > 0)
            $val = substr($amt, 0, $ind);
        return($val);
    }

    public function EncodeTime($etime) {
        $msg = "";

        $hr = $this->IntegerPart($etime / 3600);
        if ($hr > 0)
            $msg.=$hr . " Hour ";

        $HR = $etime % 3600;
        if ($HR >= 60) {
            $mn = $this->IntegerPart($HR / 60);
            if ($mn > 0)
                $msg.=$mn . " Minute ";
            $sec = $HR % 60;
            if ($sec > 0)
    ...
    public function ClearSessionVariable() {
        if (isset($_SESSION['MyDatabase']))
            unset($_SESSION['MyDatabase']);

        if (isset($_SESSION['Schema']))
            unset($_SESSION['Schema']);

        if (isset($_SESSION['Deo_Name']))
            unset($_SESSION['Deo_Name']);

        if (isset($_SESSION['Deo_Code']))
            unset($_SESSION['Deo_Code']);
        ...
        if (isset($_SESSION['LeftCount']))
            unset($_SESSION['LeftCount']);

        if (isset($_SESSION['RightCount']))
            unset($_SESSION['RightCount']);
    ...
    public function Sanitize_AllSpecial($temp, $size = 100) {
    ...
    public function Sanitize_php($temp, $size = 200) {
        if (strlen($temp) < $size)
            $size = strlen($temp);
       // echo $temp." replaced as ".
        $temp = substr($temp, 0, $size);
        $temp = str_replace("?>", "", $temp);
        $temp = str_replace("<?", "", $temp);
    ...
    public function Sanitize_Special($temp, $size = 200) {
        if (strlen($temp) < $size)
            $size = strlen($temp);
        $temp = substr($temp, 0, $size);
        $temp = str_replace("alert", "", $temp);
    ...
    public function Sanitize_Number($str, $size = 11) {
        $mstr = "";
        $a = 0;
        $str = trim($str);
        if (substr($str, 0, 1) == "-") {
        ...
        $temp = trim($minus . $mstr);
        if (strlen($temp) == 0)
            $temp = 0;
        return( $temp);
    }

    public function Sanitize_Float($str, $size = 11) {
        $mstr = "";
        $a = 0;
        $str = trim($str);
        if (substr($str, 0, 1) == "-") {
        ...
        for ($i = 0; $i < strlen($str); $i++) {
            ...
            if ($a == 0 && $i > 0 && $temp == ".") {
                ...
                $onlyonedot = false;
            if (is_numeric($temp) || $onlyonedot)
                $mstr.=$temp;
        }

        return(trim($minus . $mstr));
    }

    public function Sanitize_Space($str) {
        $mstr = "";
        for ($i = 0; $i < strlen($str); $i++) {
            $temp = substr($str, $i, 1);
            if ($this->isSpace($temp) == false)
                $mstr.=$temp;
        }
        return($mstr);
    }

    public function Sanitize_AlphaSpace($str) {
        $mstr = "";
        for ($i = 0; $i < strlen($str); $i++) {
            $temp = substr($str, $i, 1);
            if ($this->isAlphabet($temp) || $this->isSpace($temp))
                $mstr.=$temp;
        }
        return($mstr);
    }

    public function Sanitize_AlphaSpaceDot($str) {
        $mstr = "";
        ...
        for ($i = 0; $i < strlen($str); $i++) {
            $temp = substr($str, $i, 1);
            if (($this->isAlphabet($temp) || $this->isSpace($temp) || $temp == "."))
                $mstr.=$temp;
        }
        return($mstr);
    }

    public function Sanitize_Alphabet($str, $size = 20) {
        $mstr = "";

        for ($i = 0; $i < $size; $i++) {
            $temp = substr($str, $i, 1);
            if ($this->isAlphabet($temp))
                $mstr.=$temp;
        }
        return($mstr);
    }

    public function Sanitize_AlphaNumericDashDot($str, $size = 50) {
        $mstr = "";
        $firstchar = substr($str, 0, 1);
        ...
        for ($i = 0; $i < ($size); $i++) {
            $temp = substr($str, $i, 1);
           // echo $temp."<br>";
            if (is_numeric($temp) || $this->isAlphabet($temp) || $this->isSpace($temp) || $temp == "." || $temp == "-" || $temp == "_")
                $mstr.=$temp;
            //echo "converted as ".$mstr;
        }
        return($mstr);
    }

    public function Sanitize_AlphaNumeric($str, $size = 50) {
        $mstr = "";
        $firstchar = substr($str, 0, 1);
        ...
        for ($i = 0; $i < $size; $i++) {
            $temp = substr($str, $i, 1);
            if (is_numeric($temp) || $this->isAlphabet($temp))
                $mstr.=$temp;
        }
        return($mstr);
    }

    public function Sanitize_AlphaNumericSpace($str) {
        $mstr = "";
        $firstchar = substr($str, 0, 1);
        ...
        for ($i = 0; $i < strlen($str); $i++) {
            $temp = substr($str, $i, 1);
            if (is_numeric($temp) || $this->isAlphabet($temp) || $this->isSpace($temp))
                $mstr.=$temp;
        }
        return($mstr);
    }

    public function SanitizeArray($r) {
        $a = array();
        if (is_array($r))
            return($r);
        else
    ...
    public function Sanitize_ArrayElement($mvalue) {
        $tvalue = array();
        for ($i = 0; $i < count($mvalue); $i++) {
            $tvalue[$i] = $this->Sanitize_Special($mvalue[$i]);
        }
        return($tvalue);
    }

    private function isAlphabet($ch) {

        if ((ord($ch) > 64 && ord($ch) < 91) || (ord($ch) > 96 && ord($ch) < 123)) {
            //echo $ch." true ".ord($ch)."<br>"; 
            return(true);
        } else {
            //echo $ch." false  ".ord($ch)."<br>";  
            return(false);
        }
    }
...
//s alphabet
    ...
    public function IsEncryptedNumber($str) {
        $res = false;
        if (strlen($str) > 6) {
            $p1 = substr($str, 0, 3);
            $p2 = substr($str, -3);
    ...
    public function IsEncrypteText($str) {
        $res = false;
        if (substr($str, 0, 2) == "C#")
            $res = true;
        return($res);
    ...
    public function tWriteF($fname, $data) {
              //echo "Only file".$onlyfile;
            $data = $this->Sanitize_php($data);
            $data.="\n";
    ...
    public function WriteF($fname, $data) {
        $ext = explode("/", $fname);
        $cnt = count($ext) - 1;
        $onlyfile = isset($ext[$cnt]) ? $ext[$cnt] : '-';
        $onlyfile = strtolower($onlyfile);

        //echo "Only file".$onlyfile;
        if (in_array($onlyfile, $this->AllowedFile())) {
            $data = $this->Sanitize_php($data);
            $data.="\n";
            $ts = fopen($fname, 'w') or die("can't open file");
            fwrite($ts, $data);
            ...
            echo "write " . $fname;
        //else
        //  $this->alert("File Name: ".htmlentities($fname)." is not allowed");
    }
    ...
    public function AppendF($fname, $data) {
        ...
        if (in_array($onlyfile, $this->AllowedFile())) {
            ...
            echo "append " . $fname;
        // else
        //  $this->alert("File Name: ".htmlentities($fname)." is not allowed to append");
    }

    
    public function tAppendF($fname, $data) {
    ...
    public function ReadF($fname) {
        ...
        if (in_array($onlyfile, $this->AllowedFile())) {
            ...
            echo "read " . $fname;
        // else
        //$this->alert("File Name: ".htmlentities($fname)." is not allowed to read");
    }
        ...
        if (in_array($onlyfile, $this->AllowedFile())) {
            if (file_exists($filename)) {
                ...
                while (!feof($fp)) {
                    $line = fgets($fp, 1024);
                    $line = trim($line);
                    $temp[$i] = $this->Sanitize_Special($line);
                    $i++;
                }//while
            }//if
            return($temp);
        } else
            $this->alert("File Name: " . htmlentities($filename) . " is not allowed to read lines");
    }
