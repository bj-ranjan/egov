...
<script language=javascript>
    ...
    window.location="mainmenu.php";
     ...
     function back()
    {
    window.location="PRCStat.php?tag=0";
    }
    ...
    function go()
    {
        ...
        {
            myform.action="PRCStat.php?tag=1";
            myform.submit();    
        }
    }
</script>
...
//IN CASE LARGE AMOUNT OF RECORD USE MYSQL_QUERY TO FETCH RECORDS INSTEAD OF STORING IN ARRAY
    ...
    if ($tag == 0) {
        ...
        $val="";    
        ?>
        Enter Year and Petition Number for PRC separated by Comma    
...
$sql="SELECT PET_NO ,PROCESS_DATE, Applicant ,father,mother,VILLAGE,Mouza.Mouza_name , Circle.Circle  FROM PETITION_MASTER,Circle,mouza  WHERE PETITION_MASTER.Circle_code=Circle.Cir_code and PETITION_MASTER.mauza_code=mouza.Mouza_code  and  PET_YR ='".$yr."' AND PET_TYPE = 'PR' AND AST = 'Y' and Pet_no in ".$petnumbers;    

$title="Statement of PRC Certificate";
$headlist=array("Petition No","Process Date","Applicant Name","Father","Mother","Village","Mouza","Circle");
