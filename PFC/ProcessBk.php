...
function redirect(i)
{
myform.action="ProcessBk.php?tag=2&ptype=1&mtype="+i;
myform.submit();
}
...
if (parseInt(d)>=0)
{
myform.action="ProcessbK.php?tag=3";
myform.submit();
}
else
{
alert('Enter Proper  Fees') ;   
myform.Fees.focus();
}
}
else
...
if ($objUtility->checkArea($_SESSION['myArea'], 18)==false) //18 for Bakijai Certificate 
...
<tr><td colspan=2 align=Center bgcolor=#6699CC><font face=arial size=3>UPDATE BAKIJAI PETITION<br></font><font face=arial color=red size=2><?php echo  $_SESSION['msg'] ?></font></td></tr>
<?php $i=0; ?>
<tr>
...
<?php 
$objPm->setCondString(" Ast='N' and pet_type='BK'  and Pet_yr='".$mvalue[0]."' order by Pet_no desc" ); //Change the condition for where clause accordingly
$row=$objPm->getRow();
?>
...
if($fees<=0)
header('Location:ProcessBK.php?tag=1');
...
header('Location:ProcessbK.php?tag=1&mtype=100');
}//$objPm->UpdateRecord  
}


if($mtype==100)//Postback from Pet_no
echo $objUtility->alert("Updated Bakijai Fees");
