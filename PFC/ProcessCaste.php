<html>
<head>
<title>Process Caste Certificate</title>
</head>
...
myform.Caste.disabled=false;
myform.Introduced.disabled=false;
...
myform.Caste.disabled=true;
myform.Introduced.disabled=true;
...
var d=myform.Introduced.selectedIndex;
//alert(b);
//alert(c);
//alert(d);
if ( notNull(a) && validateString(a) && a_length<200 && b>0 && c>0 && d>0)
{
myform.action="ProcessCaste.php?tag=2";
myform.submit();    
}
else
...
if ($objPetition_master->getAst()=="Y" || $objPetition_master->getStatus()!="Pending" || $objPetition_master->getPet_type()!="CT")
...
$xid=0;//$objPetition_master->getXohari_requestid();
if (strlen($xid)>3)
$pno=$xid;
else
$pno=$Pet_no."/".$Pet_yr;   
...
if ($objUtility->validate($Rejected_reason,200)==true)
{
//Unicode Shouldnot Exist
...
if (isset($_POST['Introduced'])) //If HTML Field is Availbale
...
$objPetition_master->setIntroduced_by($Introduced);
...
<tr><td colspan=2 align=Center bgcolor=#ccffcc><font face=arial size=3>Successfully Processed <br>Petition No <b><?php echo $pno;?></b> for CASTE Certificate<b></font></td></tr>
<tr><td align=Center>
<?php 
...
<form name=myform action=""  method=POST >
<tr><td colspan=4 align=Center bgcolor=#ccffcc><font face=arial size=3>PROCESS CASTE CERTIFICATE<br><b><font color="#CC66FF"><?php echo  $_SESSION['msg'] ?></b></font>
...
<tr>
<td align=right bgcolor=#FFFFCC><font color=black size=2 face=arial>Introduced By</font></td><td align=left bgcolor=#FFFFCC>
<select name="Introduced" id="Introduced" style="font-family: Arial;background-color:white;color:black; font-size: 14px;font-weight:bold;width:300px" >
 <option value=""><-Select->
<option value="Chairman,SCDB">Chairman,SC Development Board
<option value="President,AJP">President,Anusuchit Jati Parishad
<option value="President,DPC">President,Dhobi People's Welfare Council
