...
function redirect(i)
{
myform.action="ProcessEroll.php?tag=2&ptype=1&mtype="+i;
myform.submit();
}
...
if (a_index>0  && b_index>0  && c_index>0 && isNumber(d))
{
if (parseInt(d)>=20)
{
myform.action="ProcessEroll.php?tag=3";
myform.submit();
}
else
{
alert('Enter Proper Jama Fees') ;   
myform.Fees.focus();
}
}
else
...
<tr><td colspan=2 align=Center bgcolor=#6699CC><font face=arial size=3>PROCESS PETITION FOR VOTER CERTIFICATE<br></font><font face=arial color=red size=2><?php echo  $_SESSION['msg'] ?></font></td></tr>
<?php $i=0; ?>
<tr>
...
<?php 
$objPm->setCondString(" Ast='N' and pet_type='ER'  and Pet_yr='".$mvalue[0]."' order by Pet_no desc" ); //Change the condition for where clause accordingly
$row=$objPm->getRow();
?>
...
<tr>
<td width="30%" align=right><font face=arial size=2>LAC and Part No</font></td>
<td width="70%" align=left><b><font face=arial size=2><?php echo $lac."(Part No-".$objPm->getPart_no() .")";?></font></td>
</tr> 
    </table>    
...
<input type=button value="Update Data"  name=Save onclick=validate()  style="font-family:arial;font-weight:bold; font-size: 14px ; background-color:#CC66FF;color:blue;width:100px">
...
if($fees<=0)
header('Location:ProcessEroll.php?tag=1');
...
header('Location:ProcessEroll.php?tag=1&mtype=100');
}//$objPm->UpdateRecord  
}


if($mtype==100)//Postback from Pet_no
echo $objUtility->alert("Updated Certificate Fees for Eroll");
