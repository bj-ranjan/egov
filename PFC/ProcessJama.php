<html>
<head>
<title>JAMABANDI Petition</title>
</head>
...
if(document.getElementById('Rej').checked==true)
{    
$("#Reason").show();
$("#O_code").hide();
$("#Fees").hide();
...
function redirect(i)
{
myform.action="ProcessJama.php?tag=2&ptype=1&mtype="+i;
myform.submit();
}
...
var rj=document.getElementById('Rej').checked;
var ok;

if(rj==true)
{
var k=document.getElementById('Reason').value; 
ok=notNull(k) && validateString(k);
}
...
ok=c_index>0 && isNumber(d) && parseInt(d)>=20;
}

if (a_index>0  && b_index>0  && ok==true)
{
myform.action="ProcessJama.php?tag=3";
myform.submit();
//alert('Ok');
}
else
...
if ($objUtility->checkArea($_SESSION['myArea'], 13)==false) //13 for Jamabandi Certificate 
...
<tr><td colspan=2 align=Center bgcolor=#6699CC><font face=arial size=3>PROCESS JAMABANDI PETITION<br></font><font face=arial color=red size=2><?php echo  $_SESSION['msg'] ?></font></td></tr>
<?php $i=0; ?>
<tr>
...
<?php 
$objPm->setCondString(" Ast='N' and pet_type='JB'  and Pet_yr='".$mvalue[0]."' order by Pet_no desc" ); //Change the condition for where clause accordingly
$row=$objPm->getRow();
?>
...
<tr>
<td width="30%" align=right><font face=arial size=2>Patta No</font></td>
<td width="70%" align=left><b><font face=arial size=2><?php echo $objPm->getPatta_no()."(".$objPm->getPatta_type().")";?></font></td>
</tr> 
<tr>
<td width="30%" align=right bgcolor="white"><font face=arial size=2>Reject Petition</font></td>
<td width="70%" align=left bgcolor="white">
<input type="checkbox" name="Rej" id="Rej">    
</td>
</tr> 
<div id="DivRej">
<tr>
<td width="30%" align=right bgcolor="white"><font face=arial size=2>Reason for Reject</font></td>
<td width="70%" align=left bgcolor="white">
...
?>
<select name=O_code id="O_code" style="font-family: Arial;background-color:white;color:black; font-size: 14px;width:200px">
<?php $dval="";?>
<option value="<?php echo $dval ;?>">-Select-
<?php 
for($ind=0;$ind<count($row);$ind++)
...
<input type=button value="Update Data"  name=Save id="Save" onclick=validate()  style="font-family:arial;font-weight:bold; font-size: 14px ; background-color:#CC66FF;color:blue;width:100px">
...
if($fees<=0)
header('Location:ProcessJama.php?tag=1');
...
$objPm->setAst("Y");
$objPm->setBo("Y");
if(isset($_POST['Rej'])) //Rejected
$objPm->setStatus("Rejected");
else
$objPm->setStatus("Processed");

if(isset($_POST['Reason']))
$objPm->setRejected_reason ($_POST['Reason']);
...
header('Location:ProcessJama.php?tag=1&mtype=100');
}//$objPm->UpdateRecord  
}


if($mtype==100)//Postback from Pet_no
echo $objUtility->alert("Updated Jamabandi");
