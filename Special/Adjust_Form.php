...
document.getElementById('Bank').value=mvalue;

var a=document.getElementById('Yr').value ;//Primary Key
var b=document.getElementById('Mn').value ;//Primary Key
var c=document.getElementById('Bank').value ;//Primary Key
if ( SimpleValidate(a,1) && isNumber(b) && SimpleValidate(c,1))
{
myform.action="Adjust_Form.php?tag=2&ptype=0";
myform.submit();
}
}
...
function redirect(i)
{
myform.action="Adjust_Form.php?tag=2&ptype=1&mtype="+i;
myform.submit();
}
...
if ( StringValid('Yr',1,1,'Yr') && NumericValid('Mn',1,'NonNegative','Mn')==true   && SelectBoxIndex('Bank')>0  &&  NumericValid('Opcase',0,'NonNegative','Opcase')==true &&  NumericValid('Amt',0,'NonNegative','Amt')==true &&  1==1)
{
...
$mvalue[0]=isset($_POST['Yr'])?$_POST['Yr']:'0';

$mvalue[1]=isset($_POST['Mn'])?$_POST['Mn']:'0';

$mvalue[2]=isset($_POST['Bank'])?$_POST['Bank']:'0';

$mvalue[3]=isset($_POST['Opcase'])?$_POST['Opcase']:'0';

$mvalue[4]=isset($_POST['Amt'])?$_POST['Amt']:'0';
...
$query="Select Bank_name,Btype from bank_master where 1=1";
//genSelectBox($id, $query, $val, $pix, $bcol, $fcol, $font, $function);
$objAdjust->TdSelectBox(1, $bcol,"Bank",$mvalue[2],$query, 160, $function);
//genSelectBoxByValueArray($id, $ValueList, $val, $pix, $bcol, $fcol, $font, $function);
echo "</tr>";
//ROW-4
echo "<tr>";
$objAdjust->bcol=$BodyColor;//Set Back Color Table Cell
$objAdjust->TdText(3, 2,"Opcase", 0, 0, 0);
$objAdjust->bcol="white";//Set Back Color for Html Box
$function=" onchange=direct1()";
//TdInputBox(1,$bcol,$id,$val,$size,$maxlength,$function,$mandatory)
...
echo "<tr>";
$objAdjust->bcol=$BodyColor;//Set Back Color Table Cell
$objAdjust->TdText(3, 2,"Amt", 0, 0, 0);
$objAdjust->bcol="white";//Set Back Color for Html Box
$function=" onchange=direct1()";
//TdInputBox(1,$bcol,$id,$val,$size,$maxlength,$function,$mandatory)
...
$query="Select Bank,Bank from Adjust where 1=1 ";
//$objAdjust->genSelectBox($id, $query, $val, $pix, $bcol, $fcol, $font, $function);
$objAdjust->genSelectBox("Editme", $query,$mvalue[5],"150", $bcol, $fcol, $font, $function);
echo "</td></tr><tr>";
echo "<td  align=left bgcolor=".$BottomColor.">";
echo "<td colspan=1 align=left bgcolor=".$BottomColor.">";
//genButton($id, $val, $pix, $bcol, $fcol, $font, $function);
$objAdjust->genButton("edit1","Edit",100 ,"#CCFFFF","black", $font," onclick=direct()");
$objAdjust->genButton("res1","Reset",100 , "#CCFFFF","black", $font," onclick=res()");
echo "</td></tr>";
...
if($mtype==1)//Postback from Yr
echo $objUtility->focus("Mn");

if($mtype==2)//Postback from Mn
echo $objUtility->focus("Bank");

if($mtype==3)//Postback from Bank
echo $objUtility->focus("Opcase");
