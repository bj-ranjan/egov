...
var url="ForwardLetter.php?id="+a;
...
myform.action="Receipt.php?tag=2&ptype=1&mtype="+i;
myform.submit();
}
...
if (SelectBoxIndex('Verification_type')>0 && StringValid('Letter_no',1,1) && DateValid('Letter_date',1) && SelectBoxIndex('Report_type')>0 && StringValid('Pol_report',0,0))
{
var name=true;    
if(ok>1)    
alert("You have Selected "+ok+" Person");
else
{
document.getElementById('SaveData').value=1;
document.getElementById('Save').disabled=true;
myform.setAttribute("target","_self")
myform.action="Receipt.php";
myform.submit();}
}
else
...
Update Report Detail Received from CO/SP<br></font><font face=arial color=red size=1>
...
$query="(Pol_status='Pending') and verification_type=".$mvalue[1];
if($mvalue[1]>1)
{
$label="Circle Name";
$query.=" and Circle=".$mvalue[2];
}
$query.=" and Dis='Y' order by Id";
...
$ref=$row[$i]['Letter_no_sent']; 
if(strlen($ref)==0)
    $ref=0;
if($ref>0)
{
$ValueList[$a][4]=$objVerification->FetchColumn("verification_letter","Letter_no","id=".$ref,"");
$date=$objVerification->FetchColumn("verification_letter","Letter_date","id=".$ref,"");
$ValueList[$a][4].="<br>Dated ".$objUtility->to_date($date);
}
else
$ValueList[$a][4]=$objUtility->to_date($row[$i]['Start_date']);
...
$headlist=array("ID","Name","Address",$label,"Ref. Letter No & Date","Select");
$align=array(2,1,1,2,1);

$objVerification->bcol=$BodyColor;//Set Back Color Table Cell
$objVerification->TdText(3, 2,"Receipt Letter No", 0, 0, 0);
$objVerification->bcol="white";//Set Back Color for Html Box
$function=" onchange=direct1()";
//TdInputBox(1,$bcol,$id,$val,$size,$maxlength,$function,$mandatory)
$objVerification->TdInputBox(1,$bcol,"Letter_no",$mvalue[3],40,100,$function,0);
echo "</tr><tr>";
$objVerification->bcol=$BodyColor;//Set Back Color Table Cell
$objVerification->TdText(3, 2,"Letter Date", 0, 0, 0);
$objVerification->bcol="white";//Set Back Color for Html Box
$function=" onchange=direct1()";
$objVerification->TdInputBoxWithDatePicker(1,$bcol,"Letter_date",$mvalue[4],10,10,$function,2);
echo "</tr>";

if($mvalue[1]==1){
echo "<tr>";    
$objVerification->bcol=$BodyColor;//Set Back Color Table Cell
$objVerification->TdText(3, 2,"Report Type", 0, 0, 0);
$objVerification->bcol="white";//Set Back Color for Html Box
$function=" onchange=direct1()";
//TdInputBox(1,$bcol,$id,$val,$size,$maxlength,$function,$mandatory)
...
$ValueL=array();
$ValueL[0][0]="Nothing Adverse";
$ValueL[1][0]="Adverse";
echo "<td>";
$objVerification->genSelectBoxByValueArray("Report_type", $ValueL, "", 160, $bcol, $fcol, $font, "");
echo "</td></tr><tr>";
$objVerification->bcol=$BodyColor;//Set Back Color Table Cell
$objVerification->TdText(3, 2,"Detail Report", 0, 0, 0);
$objVerification->bcol="white";//Set Back Color for Html Box
$function=" onchange=direct1()";
$objVerification->TdInputBox(1,$bcol,"Pol_report","",40,100,$function,0);
echo "</td></tr>";
}
echo "<tr><td>&nbsp;</td><td>";
...
$objVerification->genButton("Save", "Update Report",140 ,"#66CCCC","black", $font," onclick=validate()");
...
$mysql="update verification set ";
$maxid=$objVL->maxId();
//$objVL->setId($maxid);
//$objVL->setId_verify($myid);
//$objVL->setType($_POST['Verification_type']);
//$objVL->setFinal("Y");
$type=0;
if(isset($_POST['Verification_type']))
...
{
$mvalue[3]=trim($_POST['Letter_no']);
//$objVL->setLetter_no($mvalue[3]);
$rvalue[3]=$mvalue[3];
$mysql.=" Pol_letter_no='".$mvalue[3]."',";
}

//Push Letter_date
if (isset($_POST['Letter_date'])) //If HTML Field is Availbale
{
$mvalue[4]=trim($_POST['Letter_date']);
$rvalue[4]=$mvalue[4];
$mvalue[4]=$objUtility->to_mysqldate($mvalue[4]);
//$objVL->setLetter_date($mvalue[3]);
$mysql.=" Pol_letter_date='".$mvalue[4]."',";
}

$mysql.=" report_type='".$_POST['Report_type']."',";
$mysql.=" Pol_report ='".$_POST['Pol_report']."',";

$mysql.=" Pol_status ='Disposed' ";
$mysql.=" where Id in(".$myid.")";

//$result=$objVL->SaveRecord();
$mmode="";
$objVL->ExecuteQuery ($mysql);    
$mmode="Succesfully Updated ID ".$myid;
...
$_SESSION['msg'] = $mmode;
$msg=$mmode;
echo $objUtility->AlertNRedirect($msg, "Receipt.php?tag=1");
}//$save=1
?>
</body>
</html>
