...
<TD ALIGN=CENTER VALIGN=BOTTOM><FONT COLOR=BLUE SIZE=6 FACE="MONOTYPE CORSIVA">Dak(Search result by source)</TD></tr>
</TABLE>

<script language=javascript>
function home()
{
window.location="dakentry.php?tag=0";
}
</script>
...
session_start();
$_SESSION['searchq'] = $_POST['searchno'];
$searchno=$_SESSION['searchq'];
$i=1;
...
<td><font face=verdana size=4 color=black>Replyable(Y/N) </td>
<td><font face=verdana size=4 color=black>If Yes Target date </td>';
echo '</tr>';
if (isset($_post['search'])){
   $kkk= $_post['search'];
}
$query = mysql_query("SELECT * FROM dak_entry WHERE ltr_no LIKE '$searchno%' " ) or die("could not search");
...
$dtvarT=$row['target_date'];
$dtarrayT=array();
$dtarrayT=explode("-", $dtvarT);
$Target_dt=$dtarrayT[2]."/".$dtarrayT[1]."/".$dtarrayT[0];
